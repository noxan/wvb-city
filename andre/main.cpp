...
namespace code { // Codemodul
	...
	unsigned int values[120]; // Alle eingelesenen Farbewerte
...
int main() {  
	...
	while(true) { // Hauptschleife
		...
		//*
		speed::runSpeed(&robot, delta);
		line::runLine(&robot, delta);
		code::runCode(&robot, delta);
//
		//*/
	...
	void runCode(Robot *robot, unsigned long delta) {
		...
		if(line::status == line::FORWARD) {
			if(status == NOCODE) {
				...
				// code lesen
				while(nextTime >= 10 && index < 120) { // falls wieder Zeit ist
					nextTime -= 10; // delta
					values[index++] = raw[4];
				}
				if(index == 120) {
